...
async function deleteRoom(req, res, Room, _hotelid, id) {
...
async function getRoom(req, res, Room, id) {
...
async function getRoomData(req, res, Room, roomNumberId) {
  const one = await Room.findOne({ "roomNumbers._id": roomNumberId });
  return one ? json(req, res, 200, one) : json(req, res, 404, { error: "not found" });
}

// GET /api/v1/rooms  讀取所有房間
async function getAllRooms(req, res, Room) {
...
async function getHotelRooms(req, res, Room, hotelid) {
...
export async function roomsHandler(req, res, getMongoose) {
  ...
  const seg1 = parts[3]; // 可能是 undefined | ":id" | "find" | "findroom" | "reservartiondates" | "findHotel"
  const seg2 = parts[4]; // 可能是 ":id" 或 ":hotelid"
  const seg3 = parts[5]; // 在 /rooms/:hotelid/:id 用到
  ...
  try {
    ...
    if (req.method === "PUT" && seg1 === "reservartiondates" && seg2) {
    ...
    if (req.method === "DELETE" && seg1 && seg2 && !seg3) {
