...
/** ===================== controllers ===================== */
// POST /rooms/:hotelid  建立房間（最小實作：將 hotelId 帶到房間文件上）
...
// PUT /rooms/reservartiondates/:id  只更新不可預訂日期（最小實作：更新頂層 unavailableDates）
...
// DELETE /rooms/:hotelid/:id  刪除房間（最小實作：僅刪除 Room 文件）
...
// GET /rooms/findroom/:id  以 roomNumberID 反查 Room（常見結構：roomNumbers._id）
...
// GET /rooms/findHotel/:hotelid  讀取某飯店的所有房間（最小實作：以 room.hotelId 過濾）
...
export async function roomsHandler(req, res, getMongoose) {
  ...
  const parts = url.pathname.split("/").filter(Boolean);  // e.g. ["rooms", "find", ":id"]
